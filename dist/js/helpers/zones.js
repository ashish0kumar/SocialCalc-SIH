 ...
 *
 */
export function toZoneWithoutBoundaryChanges(xc) {
    xc = xc.split("!").pop();
 ...
 * the correct order.
 */
export function isZoneValid(zone) {
 ...
 */
export function expandZoneOnInsertion(zone, start, base, position, quantity) {
 ...
 */
export function updateSelectionOnInsertion(selection, start, base, position, quantity) {
    const dimension = start === "left" ? "columns" : "rows";
    const baseElement = position === "before" ? base - 1 : base;
    const end = start === "left" ? "right" : "bottom";
 ...
 */
export function updateSelectionOnDeletion(zone, start, elements) {
    const end = start === "left" ? "right" : "bottom";
 ...
 */
export function reduceZoneOnDeletion(zone, start, elements) {
    const end = start === "left" ? "right" : "bottom";
    let newStart = zone[start];
    let newEnd = zone[end];
    for (let removedElement of elements.sort((a, b) => b - a)) {
...
export function isInside(col, row, zone) {
 ...
 */
export function isZoneInside(smallZone, biggerZone) {
 ...
 */
export function recomputeZones(zones, toRemoveZones) {
...
export function zoneToDimension(zone) {
    return {
        height: zone.bottom - zone.top + 1,
        width: zone.right - zone.left + 1,
    };
}
export function isOneDimensional(zone) {
 ...
 */
export function positions(zone) {
    const positions = [];
...
export function createAdaptedZone(zone, dimension, operation, by) {
 ...
 * */
export function uniqueZones(zones) {
    return zones
        .reverse()
 ...
 */
export function findCellInNewZone(oldZone, currentZone) {
    let col, row;
...
export function organizeZone(zone) {
    return {
...
export function positionToZone(position) {
