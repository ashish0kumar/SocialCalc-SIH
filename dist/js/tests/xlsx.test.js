...
describe("Test XLSX export", () => {
    describe("Formula Helper : adaptFormulaToExcel", () => {
        ...
        test("composite functions", () => {
            ...
            // date as argument
            expect(adaptFormulaToExcel('=DAY(EDATE("4/5/2020",-2))')).toEqual('DAY(EDATE("2020-04-05",-2))');
            // // non-retrocompatible on Excel
    ...
    describe("Generic sheets (style, hidden, size, cf)", () => {
        ...
        test("Conditional formatting with formula cannot be exported (for now)", async () => {
            ...
            expect(await exportPrettifiedXlsx(model)).toMatchSnapshot();
            expect(console.warn).toHaveBeenCalledWith("Conditional formats with formula rules are not supported at the moment. The rule is therefore skipped.");
        });
    });
    describe("formulas", () => {
    ...
    describe("Charts", () => {
        ...
        test.each([
            ...
            ["bar", ["Sheet1!B1:B4"]],
            ["pie", ["Sheet1!B1:B4"]],
        ])("simple %s chart with dataset %s", async (chartType, dataSets) => {
    ...
    test("link cells", async () => {
        ...
        setCellContent(model, "A4", `[custom link label](${buildSheetLink(model.getters.getActiveSheetId())})`);
        expect(await exportPrettifiedXlsx(model)).toMatchSnapshot();
    });
});
describe("XML parser", () => {
    test("simple xml", () => {
        ...
        expect((_a = document.firstElementChild) === null || _a === void 0 ? void 0 : _a.outerHTML).toBe("<hello>Raoul</hello>");
    });
    test("error on the first line", () => {
        expect(() => parseXML(escapeXml `<hello>Raoul/hello>`)).toThrowError(`XML string could not be parsed: 1:19: unclosed tag: hello\n<hello>Raoul/hello>`);
    });
