...
import { addColumns, deleteColumns, merge, redo, setCellContent, undo, unMerge, } from "../test_helpers/commands_helpers";
...
describe("Multi users synchronisation", () => {
    ...
    test.each([[{ col: 0, row: 0 }], [{ col: 1, row: 0 }]])("concurrently sort and add a column in the zone with anchor %s", (anchor) => {
        ...
        expect(users).toHaveSynchronizedValue((user) => user.getters.isEmpty(sheetId, toZone("B1:B3")), true);
    });
    ...
    test("concurrently sort and remove the zone entirely", () => {
        ...
        expect(users).toHaveSynchronizedValue((user) => user.getters.isEmpty(sheetId, toZone("A1:B3")), true);
    });
    test("concurrently sort and merge cells in the zone", () => {
        ...
        expect(users).toHaveSynchronizedValue((user) => getCellContent(user, "B2"), "2");
        expect(users).toHaveSynchronizedValue((user) => getCellContent(user, "B3"), "3");
        expect(users).toHaveSynchronizedValue((user) => user.getters.isSingleCellOrMerge(sheetId, toZone("A1:A2")), true);
    });
    test("concurrently merge and sort cells in the zone", () => {
        ...
        expect(users).toHaveSynchronizedValue((user) => getCellContent(user, "B2"), "2");
        expect(users).toHaveSynchronizedValue((user) => getCellContent(user, "B3"), "3");
        expect(users).toHaveSynchronizedValue((user) => user.getters.isSingleCellOrMerge(sheetId, toZone("A1:A2")), false);
    });
...
//# sourceMappingURL=collaborative_sorting.test.js.map