...
import { addColumns, createSheet, deleteColumns, deleteRows, redo, setCellContent, snapshot, undo, } from "../test_helpers/commands_helpers";
...
describe("Collaborative local history", () => {
    ...
    test("revision id changes with each command", () => {
        expect(all).toHaveSynchronizedValue((user) => user.exportData().revisionId, DEFAULT_REVISION_ID);
        setCellContent(alice, "A1", "hello");
    ...
    test("Add a column and set a formatting on it", () => {
        ...
        expect([alice, bob, charlie]).toHaveSynchronizedValue((user) => user.getters.getCellStyle(getCell(alice, "H2")), { fillColor: "#121212" });
        expect([alice, bob, charlie]).toHaveSynchronizedValue((user) => getCell(alice, "N2"), undefined);
    });
    ...
    test("Undo or redo block the next commands until it's accepted", () => {
        ...
        network.concurrent(() => {
            undo(alice);
            expect(setCellContent(alice, "A2", "test")).toBeCancelledBecause(47 /* WaitingSessionConfirmation */);
        });
    ...
    test("Undo a create sheet command", () => {
        ...
        expect(all).toHaveSynchronizedValue((user) => getCellContent(user, "A1", sheetId), "Hello in A1");
        undo(alice);
    ...
    test("local history is cleared", () => {
        ...
        undo(alice);
        snapshot(bob);
        expect(undo(alice)).toBeCancelledBecause(5 /* EmptyUndoStack */);
        expect(redo(alice)).toBeCancelledBecause(6 /* EmptyRedoStack */);
    });
    ...
    test("snapshot is sent with a new revision id", () => {
        ...
        snapshot(alice);
        expect((_a = network.snapshot) === null || _a === void 0 ? void 0 : _a.revisionId).not.toBe(revisionId);
    });
    ...
    test("dispatch command after concurrent action with another user", () => {
        ...
        expect([alice, bob, charlie]).toHaveSynchronizedValue((user) => getCellContent(user, "A13"), "A");
        expect([alice, bob, charlie]).toHaveSynchronizedExportedData();
    });
    ...
    test("transform target command with column addition before the target edge", () => {
        ...
        expect(all).toHaveSynchronizedValue((user) => { var _a; return (_a = getCell(user, "A1")) === null || _a === void 0 ? void 0 : _a.style; }, { bold: true });
        expect(all).toHaveSynchronizedValue((user) => { var _a; return (_a = getCell(user, "B1")) === null || _a === void 0 ? void 0 : _a.style; }, undefined);
        redo(charlie);
        expect(all).toHaveSynchronizedValue((user) => { var _a; return (_a = getCell(user, "A1")) === null || _a === void 0 ? void 0 : _a.style; }, { bold: true });
        expect(all).toHaveSynchronizedValue((user) => { var _a; return (_a = getCell(user, "B1")) === null || _a === void 0 ? void 0 : _a.style; }, undefined);
        expect(all).toHaveSynchronizedExportedData();
    ...
    test("Transform command with preceding concurrent command when previous command is redone", () => {
        ...
        expect([alice, bob, charlie]).toHaveSynchronizedValue((user) => getCellContent(user, "H10"), "hello");
        redo(alice);
        expect([alice, bob, charlie]).toHaveSynchronizedValue((user) => getCellContent(user, "G10"), "hello");
        expect([alice, bob, charlie]).toHaveSynchronizedExportedData();
    });
    test("inverse delete rows then replay the command", () => {
        ...
        redo(bob);
        expect([alice, bob, charlie]).toHaveSynchronizedValue((user) => getCellContent(user, "C4"), "hello");
        expect([alice, bob, charlie]).toHaveSynchronizedExportedData();
    });
    ...
    test("redo dropped command", () => {
        ...
        expect(result).not.toBeCancelledBecause(47 /* WaitingSessionConfirmation */);
        expect(result).toBeCancelledBecause(6 /* EmptyRedoStack */);
        expect(all).toHaveSynchronizedExportedData();
    });
    ...
    test("cannot redo command previous to dropped command", () => {
        ...
        expect(redo(charlie)).toBeCancelledBecause(6 /* EmptyRedoStack */);
        expect(all).toHaveSynchronizedExportedData();
    });
});
//# sourceMappingURL=collaborative_history.test.js.map