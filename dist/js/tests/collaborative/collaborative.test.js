...
import { activateSheet, addColumns, addRows, clearCell, createChart, createSheet, deleteColumns, deleteRows, merge, moveConditionalFormat, redo, selectCell, setCellContent, undo, } from "../test_helpers/commands_helpers";
...
describe("Multi users synchronisation", () => {
    ...
    test("update two different cells concurrently", () => {
        ...
        expect([alice, bob, charlie]).toHaveSynchronizedValue((user) => getCellContent(user, "A1"), "hello in A1");
        expect([alice, bob, charlie]).toHaveSynchronizedValue((user) => getCellContent(user, "B2"), "hello in B2");
    });
    test("update the same cell concurrently", () => {
        ...
        expect([alice, bob, charlie]).toHaveSynchronizedValue((user) => getCellContent(user, "A1"), "Hi Alice");
    });
    test("update the same cell sequentially", () => {
        setCellContent(alice, "A1", "hello Bob");
        expect([alice, bob, charlie]).toHaveSynchronizedValue((user) => getCellContent(user, "A1"), "hello Bob");
        setCellContent(bob, "A1", "Hi Alice");
        expect([alice, bob, charlie]).toHaveSynchronizedValue((user) => getCellContent(user, "A1"), "Hi Alice");
    });
    test("update and delete the same cell concurrently", () => {
        setCellContent(alice, "A1", "Hi");
        expect([alice, bob, charlie]).toHaveSynchronizedValue((user) => getCellContent(user, "A1"), "Hi");
        network.concurrent(() => {
    ...
    test("Command while the previous command is not acknowledged", () => {
        ...
        expect([alice, bob, charlie]).toHaveSynchronizedValue((user) => getCellContent(user, "A1"), "hello A1");
        expect([alice, bob, charlie]).toHaveSynchronizedValue((user) => getCellContent(user, "B1"), "hello B1");
    });
    ...
    test("delete and update the same empty cell concurrently", () => {
        setCellContent(alice, "A1", "hello");
        expect([alice, bob, charlie]).toHaveSynchronizedValue((user) => getCellContent(user, "A1"), "hello");
        network.concurrent(() => {
            clearCell(alice, "A1");
    ...
    test("copy/paste on styled cell", () => {
        ...
        expect([alice, bob, charlie]).toHaveSynchronizedValue((user) => getCell(user, "B2").style, undefined);
    });
    test("Merge a cell and update a cell concurrently", () => {
        ...
        expect(undo(bob)).toBeCancelledBecause(5 /* EmptyUndoStack */);
    });
    ...
    test("merge is transformed to fit sheet size", () => {
        ...
        expect([alice, bob, charlie]).toHaveSynchronizedValue((user) => user.getters.getMerges(sheetId), [
            {
                ...toZone("A80:A98"),
                id: 1,
                topLeft: {
    ...
    test("Updatecell & composer on different cells", () => {
        ...
        expect([alice, bob, charlie]).toHaveSynchronizedValue((user) => getCellContent(user, "A2"), "A2");
        expect([alice, bob, charlie]).toHaveSynchronizedValue((user) => getCellContent(user, "A1"), "Hi");
    });
    test("Updatecell & composer on the same cell", () => {
        ...
        expect([alice, bob, charlie]).toHaveSynchronizedValue((user) => getCellContent(user, "A1"), "A1");
        alice.dispatch("STOP_EDITION");
        expect([alice, bob, charlie]).toHaveSynchronizedValue((user) => getCellContent(user, "A1"), "bla");
    });
    ...
    test("duplicate sheet does not activate sheet", () => {
        ...
        expect([alice, bob, charlie]).toHaveSynchronizedValue((user) => user.getters.getActiveSheetId(), firstSheetId);
    ...
    test("Composer is moved when column is added before it", () => {
        ...
        alice.dispatch("STOP_EDITION");
        expect([alice, bob, charlie]).toHaveSynchronizedValue((user) => getCellContent(user, "E2"), "hello");
    });
    test("Composer is not moved when column is added after it", () => {
        ...
        addColumns(bob, "after", "B", 1);
        alice.dispatch("STOP_EDITION");
        expect([alice, bob, charlie]).toHaveSynchronizedValue((user) => getCellContent(user, "A2"), "hello");
    });
    ...
    test("Composer is moved when column is removed before it", () => {
        ...
        deleteColumns(bob, ["B"]);
        alice.dispatch("STOP_EDITION");
        expect([alice, bob, charlie]).toHaveSynchronizedValue((user) => getCellContent(user, "C2"), "hello");
    });
    test("Composer is not moved when column is removed after it", () => {
        ...
        deleteColumns(bob, ["E"]);
        alice.dispatch("STOP_EDITION");
        expect([alice, bob, charlie]).toHaveSynchronizedValue((user) => getCellContent(user, "D2"), "hello");
    });
    ...
    test("Composer is moved when row is added before it", () => {
        ...
        alice.dispatch("STOP_EDITION");
        expect([alice, bob, charlie]).toHaveSynchronizedValue((user) => getCellContent(user, "A5"), "hello");
    });
    ...
    test("Composer is moved when row is removed before it", () => {
        ...
        deleteRows(bob, [1]);
        alice.dispatch("STOP_EDITION");
        expect([alice, bob, charlie]).toHaveSynchronizedValue((user) => getCellContent(user, "A3"), "hello");
    });
    test("Composer is not moved when row is removed after it", () => {
        ...
        deleteRows(bob, [10]);
        alice.dispatch("STOP_EDITION");
        expect([alice, bob, charlie]).toHaveSynchronizedValue((user) => getCellContent(user, "A4"), "hello");
    });
    ...
    test("Delete the same figure concurrently", () => {
        ...
        expect([alice, bob, charlie]).toHaveSynchronizedValue((user) => user.getters.getFigures(sheetId), [figure]);
        network.concurrent(() => {
            alice.dispatch("DELETE_FIGURE", { id: "someuuid", sheetId });
            bob.dispatch("DELETE_FIGURE", { id: "someuuid", sheetId });
        });
        expect([alice, bob, charlie]).toHaveSynchronizedValue((user) => user.getters.getFigures(sheetId), []);
    });
    ...
    test("Spreadsheet in readonly still receive commands", () => {
        ...
        setCellContent(alice, "A1", "hello");
        expect([alice, bob, charlie, david]).toHaveSynchronizedValue((user) => getCellContent(user, "A1"), "hello");
    ...
    describe("Evaluation", () => {
        ...
        test("Cell value is correctly re-evaluated after undo", () => {
            setCellContent(alice, "A1", "=5");
            expect([alice, bob, charlie]).toHaveSynchronizedValue((user) => getCell(user, "A1").evaluated.value, 5);
            setCellContent(alice, "A1", "=10");
            expect([alice, bob, charlie]).toHaveSynchronizedValue((user) => getCell(user, "A1").evaluated.value, 10);
            undo(alice);
            expect([alice, bob, charlie]).toHaveSynchronizedValue((user) => getCell(user, "A1").evaluated.value, 5);
        });
    });
    test("Reorder formatting rules concurrently", () => {
        ...
        expect([alice, bob, charlie]).toHaveSynchronizedValue((user) => user.getters.getConditionalStyle(...toCartesian("A1")), { fillColor: "#00FF00" });
        expect([alice, bob, charlie]).toHaveSynchronizedValue((user) => user.getters.getConditionalFormats(sheetId)[0].id, "3");
        expect([alice, bob, charlie]).toHaveSynchronizedValue((user) => user.getters.getConditionalFormats(sheetId)[1].id, "1");
        expect([alice, bob, charlie]).toHaveSynchronizedValue((user) => user.getters.getConditionalFormats(sheetId)[2].id, "2");
    });
    test("Reorder and delete formatting rules concurrently", () => {
        ...
        expect([alice, bob]).toHaveSynchronizedValue((user) => user.getters.getConditionalStyle(...toCartesian("A1")), { fillColor: "#FF0000" });
    });
});
//# sourceMappingURL=collaborative.test.js.map