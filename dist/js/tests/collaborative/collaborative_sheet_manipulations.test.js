...
import { activateSheet, addColumns, addRows, createChart, createSheet, deleteCells, deleteColumns, deleteRows, hideColumns, hideRows, renameSheet, selectCell, setCellContent, unhideColumns, unhideRows, updateChart, } from "../test_helpers/commands_helpers";
...
describe("Collaborative Sheet manipulation", () => {
    ...
    test("create and delete sheet concurrently", () => {
        ...
        expect([alice, bob, charlie]).toHaveSynchronizedValue((user) => user.getters.getVisibleSheets(), [sheet1, "2"]);
    ...
    test("Create two sheets concurrently", () => {
        ...
        expect([alice, bob, charlie]).toHaveSynchronizedValue((user) => user.getters.getVisibleSheets(), [sheet1, "3", "2"]);
    ...
    test("create sheet and move sheet concurrently", () => {
        ...
        expect([alice, bob, charlie]).toHaveSynchronizedValue((user) => user.getters.getVisibleSheets(), ["2", sheet1, "42"]);
        expect([alice, bob, charlie]).toHaveSynchronizedExportedData();
    });
    test("Move two sheets concurrently", () => {
        ...
        expect([alice, bob, charlie]).toHaveSynchronizedValue((user) => user.getters.getVisibleSheets(), ["1", "2", sheet1]);
        expect([alice, bob, charlie]).toHaveSynchronizedExportedData();
    });
    test("delete sheet and update figure concurrently", () => {
        ...
        expect([alice, bob, charlie]).toHaveSynchronizedValue((user) => user.getters.getFigure(sheetId, "456"), undefined);
    });
    test("delete sheet and update chart concurrently", () => {
        ...
        expect([alice, bob, charlie]).toHaveSynchronizedValue((user) => user.getters.getFigure(sheetId, chartId), undefined);
    });
    test("rename sheet and update cell with sheet ref concurrently", () => {
        ...
        expect([alice, bob, charlie]).toHaveSynchronizedValue((user) => getCellError(user, "A1"), `Invalid sheet name: ${sheetName}`);
    });
    ...
    test("Hide and add columns concurrently", () => {
        ...
        expect([alice, bob, charlie]).toHaveSynchronizedValue((user) => user.getters.getHiddenColsGroups(sheetId), toNumbers([["M"]]));
    });
    test("Hide and remove rows concurrently", () => {
        ...
        expect([alice, bob, charlie]).toHaveSynchronizedValue((user) => user.getters.getHiddenRowsGroups(sheetId), [[3]]);
    });
    test("Hide and add rows concurrently", () => {
        ...
        expect([alice, bob, charlie]).toHaveSynchronizedValue((user) => user.getters.getHiddenRowsGroups(sheetId), [[2]]);
    });
    test("Unhide and add rows concurrently", () => {
        ...
        expect([alice, bob, charlie]).toHaveSynchronizedValue((user) => user.getters.getHiddenRowsGroups(sheetId), []);
    });
    test("Unhide and remove columns concurrently", () => {
        ...
        expect([alice, bob, charlie]).toHaveSynchronizedValue((user) => user.getters.getHiddenColsGroups(sheetId), toNumbers([["G"]]));
    });
    test("Unhide and add columns concurrently", () => {
        ...
        expect([alice, bob, charlie]).toHaveSynchronizedValue((user) => user.getters.getHiddenColsGroups(sheetId), toNumbers([["D"]]));
    });
    test("Hide different rows concurrent", () => {
        ...
        expect([alice, bob, charlie]).toHaveSynchronizedValue((user) => user.getters.getHiddenRowsGroups(sheetId), [[1, 2, 3]]);
    });
    test("Unhide different rows concurrent", () => {
        ...
        expect([alice, bob, charlie]).toHaveSynchronizedValue((user) => user.getters.getHiddenRowsGroups(sheetId), [[8]]);
    });
    test("Hide and unhide columns concurrently", () => {
        const sheetId = alice.getters.getActiveSheetId();
        hideColumns(alice, ["C"], sheetId);
        expect([alice, bob, charlie]).toHaveSynchronizedValue((user) => user.getters.getHiddenColsGroups(sheetId), [[2]]);
        network.concurrent(() => {
            hideColumns(alice, ["B", "C", "D"], sheetId);
            unhideColumns(bob, ["C"]);
        });
        expect([alice, bob, charlie]).toHaveSynchronizedValue((user) => user.getters.getHiddenColsGroups(sheetId), toNumbers([["B"], ["D"]]));
    });
    test("Delete cells on columns deleted", () => {
        ...
        expect([alice, bob, charlie]).toHaveSynchronizedValue((user) => getCellContent(user, "C1"), "hello");
    });
    describe("conditional formatting", () => {
        test("Concurrent new conditional format and new columns", () => {
            ...
            expect([alice, bob, charlie]).toHaveSynchronizedValue((user) => user.getters.getConditionalFormats(sheetId), [
                {
                    id: "1",
    ...
    describe("Chart creation & update", () => {
        ...
        test(`Concurrently chart creation & update and add columns`, () => {
            ...
            expect([alice, bob, charlie]).toHaveSynchronizedValue((user) => user.getters.getChartDefinitionUI(sheetId, chartId), { ...chartDef, dataSets: ["A1:A3", "H1:H3"], labelRange: "H3" });
            network.concurrent(() => {
                addColumns(alice, "before", "D", 2);
                updateChart(bob, chartId, {
                    dataSets: ["A1:A3", "F1:F3"],
        ...
        test(`Concurrently chart creation & update and removed columns`, () => {
            ...
            expect([alice, bob, charlie]).toHaveSynchronizedValue((user) => user.getters.getChartDefinitionUI(sheetId, chartId), {
                ...chartDef,
        ...
        test(`Concurrently chart creation & update and new rows`, () => {
            ...
            network.concurrent(() => {
                addRows(alice, "before", 9, 2);
                createChart(bob, { ...chartDef, dataSets: ["A1:A3", "A4:A10", "A11:A12"], labelRange: "F10" }, chartId);
            });
            expect([alice, bob, charlie]).toHaveSynchronizedValue((user) => user.getters.getChartDefinitionUI(sheetId, chartId), { ...chartDef, dataSets: ["A1:A3", "A4:A12", "A13:A14"], labelRange: "F12" });
            network.concurrent(() => {
                addRows(alice, "before", 9, 2);
        ...
        test("Set grid lines visibility is correctly shared", () => {
            createSheet(alice, { sheetId: "42" });
            expect([alice, bob, charlie]).toHaveSynchronizedValue((user) => user.getters.getGridLinesVisibility("42"), true);
            alice.dispatch("SET_GRID_LINES_VISIBILITY", { sheetId: "42", areGridLinesVisible: false });
            expect([alice, bob, charlie]).toHaveSynchronizedValue((user) => user.getters.getGridLinesVisibility("42"), false);
        });
        test("Set grid lines visibility with a sheet deletion", () => {
            ...
            expect([alice, bob, charlie]).toHaveSynchronizedValue((user) => user.getters.tryGetSheet("42"), undefined);
        });
        test(`Concurrently chart creation & update and removed rows`, () => {
            const sheetId = alice.getters.getActiveSheetId();
            network.concurrent(() => {
                deleteRows(alice, [3, 4, 10]);
                createChart(bob, { ...chartDef, dataSets: ["A1:A3", "A4:A5", "A11:A12"], labelRange: "F10" }, chartId);
            });
            expect([alice, bob, charlie]).toHaveSynchronizedValue((user) => user.getters.getChartDefinitionUI(sheetId, chartId), {
                ...chartDef,
                dataSets: ["A1:A3", "A9"],
...
//# sourceMappingURL=collaborative_sheet_manipulations.test.js.map