...
describe("Model", () => {
    ...
    test("can add a Plugin only in headless mode", () => {
        ...
        NormalPlugin.modes = ["normal"];
        class HeadlessPlugin extends UIPlugin {
        }
        HeadlessPlugin.modes = ["headless"];
        ...
        expect(modelNormal["handlers"].find((handler) => handler instanceof HeadlessPlugin)).toBeFalsy();
        const modelHeadless = new Model({}, { mode: "headless" });
        expect(modelHeadless["handlers"].find((handler) => handler instanceof NormalPlugin)).toBeFalsy();
        expect(modelHeadless["handlers"].find((handler) => handler instanceof HeadlessPlugin)).toBeTruthy();
    });
    test("core plugin can refuse command from UI plugin", () => {
        class MyCorePlugin extends CorePlugin {
            allowDispatch(cmd) {
                if (cmd.type === "UPDATE_CELL") {
                    return 1 /* CancelledForUnknownReason */;
                }
                return 0 /* Success */;
            }
        }
        ...
        expect(result).toBeCancelledBecause(1 /* CancelledForUnknownReason */);
    ...
    test("Some commands are not dispatched in readonly mode", () => {
        const model = new Model({}, { isReadonly: true });
        expect(setCellContent(model, "A1", "hello")).toBeCancelledBecause(50 /* Readonly */);
    });
    ...
    test("Can add custom elements in the config of model", () => {
        const model = new Model({}, { custom: "42" });
