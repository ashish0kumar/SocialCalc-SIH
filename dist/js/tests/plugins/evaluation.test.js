...
import { activateSheet, createSheet, deleteColumns, setCellContent, } from "../test_helpers/commands_helpers";
...
var resetAllMocks = jest.resetAllMocks;
describe("evaluateCells", () => {
    ...
    test("error in some function calls", () => {
        ...
        expect(getCellError(model, "A1")).toBe(`The function SUM expects a number value, but 'asdf' is a string, and cannot be coerced to a number.`);
    ...
    test("error in an addition", () => {
        ...
        expect(getCellError(model, "A3")).toBe(`The function ADD expects a number value, but 'asdf' is a string, and cannot be coerced to a number.`);
    ...
    test("error in an subtraction", () => {
        ...
        setCellContent(model, "A1", "33");
        expect(getCell(model, "A3").evaluated.value).toBe("#ERROR");
        expect(getCellError(model, "A3")).toBe(`The function MINUS expects a number value, but 'asdf' is a string, and cannot be coerced to a number.`);
        setCellContent(model, "A2", "10");
