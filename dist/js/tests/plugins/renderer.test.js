import { DEFAULT_CELL_HEIGHT, DEFAULT_CELL_WIDTH, HEADER_HEIGHT, HEADER_WIDTH, MIN_CF_ICON_MARGIN, } from "../../src/constants";
import { fontSizeMap } from "../../src/fonts";
import { toZone } from "../../src/helpers";
import { Model } from "../../src/model";
import { RendererPlugin } from "../../src/plugins/ui/renderer";
import { MockCanvasRenderingContext2D } from "../setup/canvas.mock";
import { addColumns, deleteColumns, merge, resizeColumns, resizeRows, setCellContent, } from "../test_helpers/commands_helpers";
...
function getBoxFromText(model, text) {
    const rendererPlugin = getPlugin(model, RendererPlugin);
    // @ts-ignore
    return rendererPlugin.boxes.find((b) => { var _a; return ((_a = b.content) === null || _a === void 0 ? void 0 : _a.text) === text; });
}
class MockGridRenderingContext {
...
describe("renderer", () => {
    ...
    test.each(["I am a very long text", "100000000000000"])("Overflowing left-aligned content is correctly clipped", (overflowingContent) => {
        let box;
        const model = new Model({
            sheets: [
                {
    ...
    test.each([{ align: "left" }, { align: undefined }])("Overflowing number with % align is correctly clipped", (style) => {
    ...
    test.each(["I am a very long text", "100000000000000"])("Overflowing centered content is clipped on both sides", (overflowingContent) => {
        let centeredBox;
        const model = new Model({
            sheets: [
                {
    ...
    test.each(["A1", "A1:A2", "A1:A2,B1:B2", "A1,C1"])("compatible copied zones %s are all outlined with dots", (targetXc) => {
        const model = new Model();
    ...
    test.each(["A1,A2", "A1:A2,A4:A5"])("only last copied non-compatible zones %s is outlined with dots", (targetXc) => {
        const model = new Model();
        const copiedTarget = target(targetXc);
        model.dispatch("COPY", { target: copiedTarget });
        const { ctx, isDotOutlined, reset } = watchClipboardOutline(model);
