import { DEFAULT_CELL_HEIGHT, DEFAULT_CELL_WIDTH, INCORRECT_RANGE_STRING, } from "../../src/constants";
import { lettersToNumber, toXC, toZone } from "../../src/helpers";
import { Model } from "../../src/model";
import { activateSheet, addColumns, addRows, createSheet, deleteCells, deleteColumns, deleteRows, hideColumns, hideRows, insertCells, merge, redo, selectCell, setCellContent, setSelection, undo, unhideColumns, unhideRows, } from "../test_helpers/commands_helpers";
import { getBorder, getCell, getCellContent, getCellText, getMerges, } from "../test_helpers/getters_helpers";
import { getMergeCellMap, makeTestFixture, target, testUndoRedo, toPosition, XCToMergeCellMap, } from "../test_helpers/helpers";
let model;
jest.mock("../../src/helpers/uuid", () => require("../__mocks__/uuid"));
function clearColumns(indexes) {
...
describe("Clear columns", () => {
    ...
    test("cannot delete column in invalid sheet", () => {
        expect(deleteColumns(model, ["A"], "INVALID")).toBeCancelledBecause(21 /* InvalidSheetId */);
    });
});
describe("Clear rows", () => {
    ...
    test("cannot delete row in invalid sheet", () => {
        expect(deleteRows(model, [0], "INVALID")).toBeCancelledBecause(21 /* InvalidSheetId */);
    });
});
//------------------------------------------------------------------------------
// Hide/unhide
...
describe("Hide Columns", () => {
    ...
    test("Cannot hide columns on invalid sheetId", () => {
        expect(hideColumns(model, ["A"], "INVALID")).toBeCancelledBecause(21 /* InvalidSheetId */);
    });
});
describe("Hide Rows", () => {
    const sheetId = "2";
    ...
    test("Cannot hide rows on invalid sheetId", () => {
        expect(hideRows(model, [0], "INVALID")).toBeCancelledBecause(21 /* InvalidSheetId */);
    });
});
//------------------------------------------------------------------------------
// Columns
...
describe("Columns", () => {
    ...
    describe("Correctly update size, name, order and number", () => {
        ...
        test("On addition in invalid sheet", () => {
            ...
            expect(addColumns(model, "after", "A", 1, sheetId)).toBeCancelledBecause(21 /* InvalidSheetId */);
        });
    ...
    describe("Correctly update merges", () => {
        ...
        test("On deletion", () => {
            ...
            expect(getMerges(model)).toEqual({
                1: { id: 1, topLeft: toPosition("A1"), top: 0, bottom: 0, left: 0, right: 2 },
                2: { id: 2, topLeft: toPosition("B2"), top: 1, bottom: 1, left: 1, right: 2 },
                3: { id: 3, topLeft: toPosition("B3"), top: 2, bottom: 2, left: 1, right: 2 },
            });
            expect(getMergeCellMap(model)).toEqual(XCToMergeCellMap(model, ["A1", "B1", "C1", "B2", "C2", "B3", "C3"]));
        });
        test("On addition", () => {
            addColumns(model, "before", "B", 1);
            addColumns(model, "after", "A", 1);
            expect(getMerges(model)).toEqual({
                1: { id: 1, topLeft: toPosition("A1"), top: 0, bottom: 0, left: 0, right: 6 },
                2: { id: 2, topLeft: toPosition("D2"), top: 1, bottom: 1, left: 3, right: 6 },
                3: { id: 3, topLeft: toPosition("E3"), top: 2, bottom: 2, left: 4, right: 6 },
                4: { id: 4, topLeft: toPosition("D4"), top: 3, bottom: 3, left: 3, right: 5 },
            });
            // prettier-ignore
            expect(getMergeCellMap(model)).toEqual(XCToMergeCellMap(model, [
    ...
    describe("Correctly update references", () => {
        ...
        test("update cross sheet range on column deletion in inactive sheet", () => {
            model = new Model({
                sheets: [
                    { name: "Sheet0", colNumber: 1, rowNumber: 1 },
...
describe("Rows", () => {
    ...
    describe("Correctly update size, name, order and number", () => {
        ...
        test("cannot delete column in invalid sheet", () => {
            const sheetId = "invalid";
            expect(addRows(model, "after", 0, 1, sheetId)).toBeCancelledBecause(21 /* InvalidSheetId */);
        });
    ...
    describe("Correctly update merges", () => {
        ...
        test("On deletion", () => {
            ...
            expect(getMerges(model)).toEqual({
                1: { id: 1, topLeft: toPosition("A1"), top: 0, bottom: 2, left: 0, right: 0 },
                2: { id: 2, topLeft: toPosition("B2"), top: 1, bottom: 2, left: 1, right: 1 },
                3: { id: 3, topLeft: toPosition("C2"), top: 1, bottom: 2, left: 2, right: 2 },
            });
            expect(getMergeCellMap(model)).toEqual(XCToMergeCellMap(model, ["A1", "A2", "A3", "B2", "B3", "C2", "C3"]));
        });
        test("On addition", () => {
            addRows(model, "before", 1, 1);
            addRows(model, "after", 0, 1);
            expect(getMerges(model)).toEqual({
                1: { id: 1, topLeft: toPosition("A1"), top: 0, bottom: 6, left: 0, right: 0 },
                2: { id: 2, topLeft: toPosition("B4"), top: 3, bottom: 6, left: 1, right: 1 },
                3: { id: 3, topLeft: toPosition("C5"), top: 4, bottom: 6, left: 2, right: 2 },
                4: { id: 4, topLeft: toPosition("D4"), top: 3, bottom: 5, left: 3, right: 3 },
            });
            // prettier-ignore
            expect(getMergeCellMap(model)).toEqual(XCToMergeCellMap(model, [
                "A1",
...
describe("Insert/Delete cells with merge", () => {
    test("Insert/Delete cell is rejected if a merge is blocking left-right", () => {
        model = new Model();
        merge(model, "B1:B2");
        expect(deleteCells(model, "A1", "left")).toBeCancelledBecause(2 /* WillRemoveExistingMerge */);
        expect(insertCells(model, "A1", "right")).toBeCancelledBecause(2 /* WillRemoveExistingMerge */);
    });
    test("Insert/Delete cell is rejected if a merge is blocking up-down", () => {
        model = new Model();
        merge(model, "A2:B2");
        expect(deleteCells(model, "A1", "up")).toBeCancelledBecause(2 /* WillRemoveExistingMerge */);
        expect(insertCells(model, "A1", "down")).toBeCancelledBecause(2 /* WillRemoveExistingMerge */);
    });
});
//# sourceMappingURL=grid_manipulation.test.js.map