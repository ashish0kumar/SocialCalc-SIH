...
import { activateSheet, addColumns, addRows, deleteColumns, deleteRows, hideColumns, hideRows, merge, moveAnchorCell, redo, resizeColumns, resizeRows, selectAll, selectCell, selectColumn, selectRow, setSelection, undo, } from "../test_helpers/commands_helpers";
let model;
function getMaximumOffsets(model) {
    const sheet = model.getters.getActiveSheet();
    const { height: gridHeight, width: gridWidth } = model.getters.getMaxViewportSize(sheet);
    const { height: viewportHeight, width: viewportWidth } = model.getters.getViewportDimensionWithHeaders();
    return {
...
describe("Viewport of Simple sheet", () => {
    ...
    test("cannot set offset outside of the grid", () => {
        ...
        expect(negativeOffsetResult).toBeCancelledBecause(51 /* InvalidOffset */);
        // too large
        ...
        expect(tooLargeOffsetResult).toBeCancelledBecause(51 /* InvalidOffset */);
    });
    ...
    test("Resize (reduce) columns correctly changes offset", () => {
        ...
        selectCell(model, "Z1");
        selectAll(model);
        resizeColumns(model, [...Array(cols.length).keys()].map(numberToLetters), DEFAULT_CELL_WIDTH / 2);
    ...
    test("Resizing the viewport impacts current Offset", () => {
        ...
        let { width: gridWidth, height: gridHeight } = model.getters.getMaxViewportSize(model.getters.getActiveSheet());
        ...
        ({ width: gridWidth, height: gridHeight } = model.getters.getMaxViewportSize(model.getters.getActiveSheet()));
        expect(model.getters.getActiveViewport()).toMatchObject({
...
describe("shift viewport up/down", () => {
    ...
    test.each(["A1", "A2"])("viewport and selection %s do not move when its already the end of the sheet", (selectedCell) => {
        const sheetId = model.getters.getActiveSheetId();
    ...
    test.each(["A1", "A2", "A15"])("anchor %s is shifted by the correct amount when the sheet end is reached", (selectedCell) => {
        const { bottom } = model.getters.getActiveViewport();
        const sheetId = model.getters.getActiveSheetId();
