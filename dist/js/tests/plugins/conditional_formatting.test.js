...
import { activateSheet, addColumns, addRows, createSheet, deleteCells, deleteColumns, deleteRows, moveConditionalFormat, redo, setCellContent, undo, } from "../test_helpers/commands_helpers";
...
describe("conditional format", () => {
    ...
    test("add conditional format outside the sheet", () => {
        ...
        createSheet(model, { sheetId: "42" });
        const sheetId = model.getters.getActiveSheetId();
        expect(model.dispatch("ADD_CONDITIONAL_FORMAT", {
            cf: createEqualCF("4", { fillColor: "#0000FF" }, "2"),
            target: [toZone("A1:A4000")],
            sheetId: sheetId,
        })).toBeCancelledBecause(16 /* TargetOutOfSheet */);
    });
    ...
    test("Conditional formatting with empty target", () => {
        ...
        expect(result).toBeCancelledBecause(19 /* EmptyRange */);
    ...
    test("is saved/restored", () => {
        ...
        const sheetId = model.getters.getActiveSheetId();
        expect(newModel.getters.getConditionalFormats(sheetId)).toEqual(model.getters.getConditionalFormats(sheetId));
    });
...
describe("conditional formats types", () => {
    describe("CellIs condition", () => {
        ...
        test.each([
            ...
            ["Between", ["1", "1"]],
            ["NotBetween", ["1", "1"]],
        ])("%s operator with valid number of arguments: %s", (operator, values) => {
            let result = model.dispatch("ADD_CONDITIONAL_FORMAT", {
                cf: {
                    rule: {
                        type: "CellIsRule",
        ...
        test.each([
            ...
            ["Between", ["", "1"]],
            ["NotBetween", ["", "1"]],
        ])("%s operator with missing first argument %s", (operator, values) => {
            let result = model.dispatch("ADD_CONDITIONAL_FORMAT", {
                cf: {
                    rule: {
                        type: "CellIsRule",
            ...
            expect(result).toBeCancelledBecause(34 /* FirstArgMissing */);
        });
    });
    test.each([
        ["Between", ["1"]],
        ...
        ["NotBetween", ["1"]],
        ["NotBetween", ["1", ""]],
    ])("%s operator with missing second argument %s", (operator, values) => {
        let result = model.dispatch("ADD_CONDITIONAL_FORMAT", {
            cf: {
                rule: {
                    type: "CellIsRule",
        ...
        expect(result).toBeCancelledBecause(35 /* SecondArgMissing */);
    });
    test.each([
        ["Between", ["", ""]],
        ["NotBetween", ["", ""]],
    ])("%s operator with both arguments missing %s", (operator, values) => {
        let result = model.dispatch("ADD_CONDITIONAL_FORMAT", {
            cf: {
                rule: {
                    type: "CellIsRule",
        ...
        expect(result).toBeCancelledBecause(34 /* FirstArgMissing */, 35 /* SecondArgMissing */);
    });
    describe("Icon set", () => {
        describe.each(["", "aaaa", "=SUM(1, 2)"])("dispatch is not allowed if value is not a number", (value) => {
            test("lower inflection point is NaN", () => {
                ...
                expect(result).toBeCancelledBecause(39 /* ValueUpperInflectionNaN */);
            });
            test("upper inflection point is NaN", () => {
                ...
                expect(result).toBeCancelledBecause(40 /* ValueLowerInflectionNaN */);
            });
        });
        test("refuse invalid and async formulas %s", () => {
            ...
            expect(result).toBeCancelledBecause(45 /* ValueLowerInvalidFormula */, 44 /* ValueUpperInvalidFormula */);
        });
        describe.each([
            ["number", "number"],
            ["percentage", "percentage"],
            ["percentile", "percentile"],
        ])("dispatch is not allowed if points not ascending (upper: %s , lower: %s)", (lowerInflectionPoint, upperInflectionPoint) => {
            test("upper bigger than lower", () => {
                ...
                expect(result).toBeCancelledBecause(31 /* LowerBiggerThanUpper */);
            });
        });
        test("single cell", () => {
            setCellContent(model, "A1", "5");
        ...
        test.each(["hello", "TRUE", "=TRUE", `="hello"`, ""])("is not applied if cell is not a number: %s", (content) => {
    ...
    describe("color scale", () => {
        describe.each(["", "aaaa", "=SUM(1, 2)"])("dispatch is not allowed if value is not a number", (value) => {
            test("minimum is NaN", () => {
                ...
                expect(result).toBeCancelledBecause(36 /* MinNaN */);
            });
            test("midpoint is NaN", () => {
                ...
                expect(result).toBeCancelledBecause(37 /* MidNaN */);
            });
            test("maximum is NaN", () => {
                const result = model.dispatch("ADD_CONDITIONAL_FORMAT", {
                    sheetId: model.getters.getActiveSheetId(),
                ...
                expect(result).toBeCancelledBecause(38 /* MaxNaN */);
            });
        });
        describe.each([
        ...
        ])("dispatch is not allowed if points not ascending (min: %s , mid: %s, max: %s )", (minType, midType, maxType) => {
            test("min bigger than max", () => {
                ...
                expect(result).toBeCancelledBecause(30 /* MinBiggerThanMax */);
            });
            test("mid bigger than max", () => {
                ...
                expect(result).toBeCancelledBecause(32 /* MidBiggerThanMax */);
            });
            test("min bigger than mid", () => {
                ...
                expect(result).toBeCancelledBecause(33 /* MinBiggerThanMid */);
            });
        });
        test("1 point, value scale", () => {
            setCellContent(model, "A1", "10");
            model.dispatch("ADD_CONDITIONAL_FORMAT", {
                cf: createColorScale("1", { type: "value", color: 0xff00ff }, { type: "value", color: 0x123456 }),
                target: [toZone("A1")],
                sheetId: model.getters.getActiveSheetId(),
            });
            expect(model.getters.getConditionalStyle(...toCartesian("A1"))).toEqual(undefined);
        ...
        test("2 points, value scale with any value 0", () => {
            ...
            setCellContent(model, "A3", "5");
            model.dispatch("ADD_CONDITIONAL_FORMAT", {
                cf: createColorScale("1", { type: "value", color: 0xff0000 }, { type: "value", color: 0x00ff00 }),
                target: [toZone("A1:A5")],
                sheetId: model.getters.getActiveSheetId(),
            });
            expect(model.getters.getConditionalStyle(...toCartesian("A1"))).toEqual({
