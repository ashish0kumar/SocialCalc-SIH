...
import { activateSheet, addCellToSelection, addColumns, addRows, createSheet, redo, resizeColumns, resizeRows, selectCell, setAnchorCorner, setCellContent, setSelection, undo, } from "../test_helpers/commands_helpers";
import { getCell, getCellContent, getCellError, getRangeFormattedValues, getRangeValues, } from "../test_helpers/getters_helpers";
describe("core", () => {
    describe("statistic functions", () => {
        ...
        describe("raise error from compilation with specific error message", () => {
            ...
            expect(getCellError(model, "A1")).toBe(`Invalid number of arguments for the TWOARGSNEEDED function. Expected 2 minimum, but got 1 instead.`);
        });
    });
    describe("format", () => {
    ...
    test("single cell XC conversion", () => {
        const model = new Model({});
        expect(model.getters.zoneToXC(model.getters.getActiveSheetId(), 
        /*A1*/ { top: 0, left: 0, right: 0, bottom: 0 })).toBe("A1");
    });
    test("multi cell zone XC conversion", () => {
        const model = new Model({});
        expect(model.getters.zoneToXC(model.getters.getActiveSheetId(), 
        /*A1:B2*/ { top: 0, left: 0, right: 1, bottom: 1 })).toBe("A1:B2");
    });
    test("xc is expanded to overlapping merges", () => {
        ...
        expect(model.getters.zoneToXC(model.getters.getActiveSheetId(), 
        /*A2:B3*/ { top: 1, bottom: 2, left: 0, right: 1 })).toBe("A1:B3");
    });
    test("zone is across two merges", () => {
        ...
        expect(model.getters.zoneToXC(model.getters.getActiveSheetId(), 
        /*A2:B4*/ { top: 1, bottom: 3, left: 0, right: 1 })).toBe("A1:B5");
    });
    test("merge is considered as one single cell", () => {
        ...
        expect(model.getters.zoneToXC(model.getters.getActiveSheetId(), 
        /*A2:B2*/ { top: 1, bottom: 1, left: 0, right: 1 })).toBe("A1");
    });
...
describe("history", () => {
    ...
    test("Cannot update a cell in invalid sheet", async () => {
        const model = new Model();
        expect(setCellContent(model, "B2", "hello", "invalid")).toBeCancelledBecause(21 /* InvalidSheetId */);
    });
