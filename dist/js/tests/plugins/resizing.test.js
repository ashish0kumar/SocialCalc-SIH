...
import { activateSheet, createSheet, redo, resizeColumns, resizeRows, undo, } from "../test_helpers/commands_helpers";
describe("Model resizer", () => {
    ...
    test("Cannot resize column in invalid sheet", async () => {
        const model = new Model();
        expect(resizeColumns(model, ["B"], 100, "invalid")).toBeCancelledBecause(21 /* InvalidSheetId */);
    });
    test("Cannot resize row in invalid sheet", async () => {
        const model = new Model();
        expect(resizeRows(model, [1], 100, "invalid")).toBeCancelledBecause(21 /* InvalidSheetId */);
    });
    test("Cannot auto resize column in invalid sheet", async () => {
        const model = new Model();
        expect(model.dispatch("AUTORESIZE_COLUMNS", {
            sheetId: "invalid",
            cols: [10],
    ...
    test("Cannot auto resize row in invalid sheet", async () => {
        const model = new Model();
        expect(model.dispatch("AUTORESIZE_ROWS", {
            sheetId: "invalid",
            rows: [10],
        })).toBeCancelledBecause(21 /* InvalidSheetId */);
    });
    ...
    test("changing sheets update the sizes", async () => {
        ...
        expect(model.getters.getMaxViewportSize(model.getters.getActiveSheet()).width).toBe(initialWidth - 100);
    });
...
//# sourceMappingURL=resizing.test.js.map