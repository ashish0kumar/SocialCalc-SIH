...
import { activateSheet, addCellToSelection, addColumns, createSheet, deleteColumns, hideColumns, hideRows, moveAnchorCell, redo, resizeAnchorZone, selectAll, selectCell, selectColumn, selectRow, setAnchorCorner, setSelection, undo, } from "../test_helpers/commands_helpers";
...
describe("selection", () => {
    ...
    test("cannot select out of bound row", () => {
        ...
        expect(selectRow(model, -1, "overrideSelection")).toBeCancelledBecause(15 /* SelectionOutOfBound */);
        expect(selectRow(model, 11, "overrideSelection")).toBeCancelledBecause(15 /* SelectionOutOfBound */);
    });
    test("cannot select out of bound column", () => {
        ...
        expect(selectColumn(model, -1, "overrideSelection")).toBeCancelledBecause(15 /* SelectionOutOfBound */);
        expect(selectColumn(model, 11, "overrideSelection")).toBeCancelledBecause(15 /* SelectionOutOfBound */);
    });
...
describe("Alter selection starting from hidden cells", () => {
    test("Cannot change selection if the current one is completely hidden", () => {
        ...
        expect(alter1).toBeCancelledBecause(15 /* SelectionOutOfBound */);
        const alter2 = resizeAnchorZone(model, 1, 0);
        expect(alter2).toBeCancelledBecause(15 /* SelectionOutOfBound */);
    });
    ...
    test.each([
        ...
        [["Y", "Z"], "Z1", 1, "X1:Z1"],
        [["Y", "Z"], "Y1", 1, "X1:Y1"],
    ])("Alter selection horizontally from hidden col", (hiddenCols, startPosition, delta, endPosition) => {
        const model = new Model();
    ...
    test.each([
        ...
        [["A", "B"], "A1:B1", 1, "A1:B1"],
        [["A", "C"], "A1:C1", 1, "A1:C2"],
    ])("Alter selection vertically from hidden col needs at least one visible selected cell", (hiddenCols, startPosition, delta, endPosition) => {
        const model = new Model();
    ...
    test.each([
        ...
        [[98, 99], "A100", 1, "A98:A100"],
        [[98, 99], "A99", 1, "A98:A99"],
    ])("Alter selection vertically from hidden col", (hiddenRows, startPosition, delta, endPosition) => {
        const model = new Model();
        selectCell(model, startPosition);
    ...
    test.each([
        ...
        [[0, 1], "A1:A2", 1, "A1:A2"],
        [[0, 2], "A1:A3", 1, "A1:B3"],
    ])("Alter selection horizontally from hidden col", (hiddenRows, startPosition, delta, endPosition) => {
        const model = new Model();
        setSelection(model, [startPosition]);
        hideRows(model, hiddenRows);
        resizeAnchorZone(model, delta, 0);
...
describe("move elements(s)", () => {
    ...
    test("can't move columns whose merges overflow from the selection", () => {
        ...
        expect(result).toBeCancelledBecause(2 /* WillRemoveExistingMerge */);
    });
