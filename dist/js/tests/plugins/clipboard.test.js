...
import { CellValueType } from "../../src/types/index";
import { activateSheet, addCellToSelection, createSheet, createSheetWithName, deleteColumns, deleteRows, selectCell, setAnchorCorner, setCellContent, undo, } from "../test_helpers/commands_helpers";
import { getBorder, getCell, getCellContent, getCellError, getCellText, } from "../test_helpers/getters_helpers";
import { createEqualCF, getGrid, getPlugin, makeInteractiveTestEnv, target, } from "../test_helpers/helpers";
function getClipboardVisibleZones(model) {
...
describe("clipboard", () => {
    ...
    test("paste without copied value", () => {
        ...
        expect(result).toBeCancelledBecause(18 /* EmptyClipboard */);
    });
    ...
    test("can copy and paste merged content", () => {
        ...
        expect(model.getters.isInMerge(model.getters.getActiveSheetId(), ...toCartesian("B4"))).toBe(true);
        expect(model.getters.isInMerge(model.getters.getActiveSheetId(), ...toCartesian("B5"))).toBe(true);
        expect(model.getters.isInMerge(model.getters.getActiveSheetId(), ...toCartesian("C4"))).toBe(true);
        expect(model.getters.isInMerge(model.getters.getActiveSheetId(), ...toCartesian("B5"))).toBe(true);
    });
    ...
    test("pasting numbers from windows clipboard => interpreted as number", () => {
        ...
        expect((_a = getCell(model, "C1")) === null || _a === void 0 ? void 0 : _a.evaluated.value).toBe(1);
        expect(getCellContent(model, "C2")).toBe("2");
        expect((_b = getCell(model, "C2")) === null || _b === void 0 ? void 0 : _b.evaluated.value).toBe(2);
        expect(getCellContent(model, "C3")).toBe("3");
        expect((_c = getCell(model, "C3")) === null || _c === void 0 ? void 0 : _c.evaluated.value).toBe(3);
    });
    ...
    test("pasting is not allowed if multiple selection and more than one value", () => {
        ...
        expect(result).toBeCancelledBecause(17 /* WrongPasteSelection */);
    });
