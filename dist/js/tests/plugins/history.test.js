...
import { activateSheet, createSheet, redo, selectCell, setCellContent, undo, } from "../test_helpers/commands_helpers";
...
describe("Model history", () => {
    ...
    test("undo when undo stack is empty does nothing", async () => {
        ...
        expect(undo(model)).toBeCancelledBecause(5 /* EmptyUndoStack */);
        expect(getCell(model, "A1").evaluated.value).toBe(10);
    });
    test("undo when redo stack is empty does nothing", async () => {
        const model = new Model({ sheets: [{ cells: { A1: { content: "=10" } } }] });
        expect(getCell(model, "A1").evaluated.value).toBe(10);
        expect(redo(model)).toBeCancelledBecause(6 /* EmptyRedoStack */);
        expect(getCell(model, "A1").evaluated.value).toBe(10);
    });
