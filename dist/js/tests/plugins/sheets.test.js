...
import { activateSheet, createChart, createSheet, createSheetWithName, deleteRows, hideRows, merge, redo, renameSheet, resizeColumns, resizeRows, setCellContent, undo, unMerge, } from "../test_helpers/commands_helpers";
...
describe("sheets", () => {
    ...
    test("Cannot create a sheet with a name already existent", () => {
        ...
        expect(createSheetWithName(model, {
            sheetId: "42",
            position: 1,
        }, name)).toBeCancelledBecause(10 /* DuplicatedSheetName */);
    });
    test("Cannot create a sheet with a name already existent + upper", () => {
        ...
        expect(createSheetWithName(model, {
            sheetId: "42",
            position: 1,
        }, name.toUpperCase())).toBeCancelledBecause(10 /* DuplicatedSheetName */);
    });
    ...
    test.each(FORBIDDEN_SHEET_CHARS)("Cannot rename a sheet with a %s in the name", (char) => {
        const model = new Model();
        expect(renameSheet(model, model.getters.getActiveSheetId(), `my life ${char}`)).toBeCancelledBecause(11 /* ForbiddenCharactersInSheetName */);
    });
    test("Cannot create a sheet with a position > length of sheets", () => {
        const model = new Model();
        expect(model.dispatch("CREATE_SHEET", { sheetId: "42", position: 54 })).toBeCancelledBecause(13 /* WrongSheetPosition */);
    });
    test("Cannot create a sheet with a negative position", () => {
        const model = new Model();
        expect(model.dispatch("CREATE_SHEET", { sheetId: "42", position: -1 })).toBeCancelledBecause(13 /* WrongSheetPosition */);
    });
    ...
    test("Cannot delete an invalid sheet", async () => {
        const model = new Model();
        expect(model.dispatch("DELETE_SHEET", { sheetId: "invalid" })).toBeCancelledBecause(21 /* InvalidSheetId */);
    });
    ...
    test("cannot activate an invalid sheet", () => {
        const model = new Model();
        expect(activateSheet(model, "INVALID_ID")).toBeCancelledBecause(21 /* InvalidSheetId */);
    });
    ...
    test("cannot move the first sheet to left and the last to right", () => {
        ...
        const sheet1 = model.getters.getVisibleSheets()[0];
        const sheet2 = model.getters.getVisibleSheets()[1];
        expect(model.dispatch("MOVE_SHEET", { sheetId: sheet1, direction: "left" })).toBeCancelledBecause(12 /* WrongSheetMove */);
        expect(model.dispatch("MOVE_SHEET", { sheetId: sheet2, direction: "right" })).toBeCancelledBecause(12 /* WrongSheetMove */);
    });
    ...
    test("Cannot rename an invalid sheet", async () => {
        const model = new Model();
        expect(renameSheet(model, "invalid", "hello")).toBeCancelledBecause(21 /* InvalidSheetId */);
    });
    ...
    test("Cannot rename a sheet with existing name", () => {
        ...
        expect(renameSheet(model, sheet, name)).toBeCancelledBecause(10 /* DuplicatedSheetName */);
        expect(renameSheet(model, sheet, "new_name")).toBeCancelledBecause(10 /* DuplicatedSheetName */);
        expect(renameSheet(model, sheet, "new_name ")).toBeCancelledBecause(10 /* DuplicatedSheetName */);
    });
    test("Cannot rename a sheet without name", () => {
        ...
        renameSheet(model, sheet, undefined)).toBeCancelledBecause(9 /* MissingSheetName */);
        expect(renameSheet(model, sheet, "    ")).toBeCancelledBecause(9 /* MissingSheetName */);
    });
    ...
    test("Cannot delete sheet if there is only one", () => {
        const model = new Model();
        expect(model.dispatch("DELETE_SHEET", { sheetId: model.getters.getActiveSheetId() })).toBeCancelledBecause(8 /* NotEnoughSheets */);
    });
    ...
    test("Sheet reference are correctly marked as #REF on sheet deletion", () => {
        ...
        model.dispatch("DELETE_SHEET", { sheetId: sheet2 });
        expect(getCellText(model, "A1")).toBe("=NEW_NAME!A1");
        expect((_a = getCell(model, "A1")) === null || _a === void 0 ? void 0 : _a.evaluated.value).toBe("#ERROR");
        undo(model);
        activateSheet(model, sheet1);
        expect(getCellText(model, "A1")).toBe("=NEW_NAME!A1");
        expect((_b = getCell(model, "A1")) === null || _b === void 0 ? void 0 : _b.evaluated.value).toBe(42);
    });
    ...
    test("Cannot remove more columns/rows than there are inside the sheet", () => {
        ...
        expect(deleteRows(model, [1, 2, 3, 4])).toBeCancelledBecause(7 /* NotEnoughElements */);
    });
    test("Cannot have all rows/columns hidden at once", () => {
        ...
        expect(hideRows(model, [0, 1, 3])).toBeCancelledBecause(49 /* TooManyHiddenElements */);
    });
    ...
    test("Dispatch set the grid lines visibility on invalid sheet", () => {
        ...
        expect(model.dispatch("SET_GRID_LINES_VISIBILITY", { sheetId, areGridLinesVisible: false })).toBeCancelledBecause(21 /* InvalidSheetId */);
    });
