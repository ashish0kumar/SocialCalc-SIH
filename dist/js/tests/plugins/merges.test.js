...
import { addColumns, deleteRows, deleteSheet, interactiveMerge, merge, moveAnchorCell, redo, selectCell, setAnchorCorner, setCellContent, undo, unMerge, } from "../test_helpers/commands_helpers";
import { simulateClick } from "../test_helpers/dom_helper";
import { getActiveXc, getBorder, getCell, getCellContent, getMerges, } from "../test_helpers/getters_helpers";
import { getChildFromComponent, getMergeCellMap, makeTestFixture, target, toPosition, XCToMergeCellMap, } from "../test_helpers/helpers";
function getCellsXC(model) {
...
describe("merges", () => {
    ...
    test("add a merge cells in a duplicated sheet", () => {
        ...
        expect((_a = model.getters.getMerge(secondSheetId, 2, 1)) === null || _a === void 0 ? void 0 : _a.id).toBe(2);
        expect((_b = model.getters.getMerge(secondSheetId, 1, 1)) === null || _b === void 0 ? void 0 : _b.id).toBe(3);
    });
    ...
    test("merge outside the sheet is refused", () => {
        ...
        expect(merge(model, "A1:C3")).toBeCancelledBecause(16 /* TargetOutOfSheet */);
    ...
    test("Merge with two zone overlap is now allowed", () => {
        const model = new Model();
        const sheetId = model.getters.getActiveSheetId();
        expect(model.dispatch("ADD_MERGE", { sheetId, target: [toZone("A1:B2"), toZone("A2:B3")] })).toBeCancelledBecause(48 /* MergeOverlap */);
    });
    test("properly compute if a merge is destructive or not", () => {
        ...
        expect(interactiveMerge(model, "A1:C4")).toBeCancelledBecause(3 /* MergeIsDestructive */);
    ...
    test("merging destructively a selection ask for confirmation", async () => {
        ...
        Parent.template = xml /* xml */ `<Spreadsheet model="_model"/>`;
        Parent.components = { Spreadsheet };
    ...
    test("Cannot add a merge in a non-existing sheet", () => {
        const model = new Model();
        expect(merge(model, "A1:A2", "invalid")).toBeCancelledBecause(21 /* InvalidSheetId */);
    });
    ...
    test("update content cell of merged cell, other than top left", () => {
        const model = new Model();
        merge(model, "A1:A2");
        expect(setCellContent(model, "A2", "hello")).toBeCancelledBecause(4 /* CellIsMerged */);
        expect(getCell(model, "A2")).toBeUndefined();
    });
