...
import { activateSheet, addCellToSelection, createSheet, createSheetWithName, merge, resizeAnchorZone, selectCell, setAnchorCorner, setSelection, } from "../test_helpers/commands_helpers";
function select(model, xc) {
    selectCell(model, xc);
    model.dispatch("STOP_SELECTION_INPUT");
}
function highlightedZones(model) {
    return model.getters
        .getHighlights()
        .map((h) => h.zone)
        .map(zoneToXc);
}
function idOfRange(model, id, rangeIndex) {
...
describe("selection input plugin", () => {
    ...
    test("focus input which is already focused", () => {
        ...
        expect(result).toBeCancelledBecause(22 /* InputAlreadyFocused */);
    });
    ...
    test("cannot add empty range when maximum ranges reached", () => {
        model.dispatch("ENABLE_NEW_SELECTION_INPUT", { id, hasSingleRange: true });
        expect(model.getters.getSelectionInput(id)).toHaveLength(1);
        expect(model.dispatch("ADD_EMPTY_RANGE", { id })).toBeCancelledBecause(23 /* MaximumRangesReached */);
    });
    ...
    test.each(["sheet name", "Sheet+", "Sheet:)"])("can select a range  with special characters in its name: %s", (sheetName) => {
        model.dispatch("ENABLE_NEW_SELECTION_INPUT", { id });
