...
describe("expression compiler", () => {
    ...
    test.each(["=1 + 3", "=2 * 3", "=2 - 3", "=2 / 3", "=-3", "=(3 + 1) * (-1 + 4)"])("some arithmetic expressions", (formula) => {
        const compiledFormula = compiledBaseFunction(formula);
        expect(compiledFormula.execute.toString()).toMatchSnapshot();
    });
    test.each(["=sum(1,2)", '=sum(true, "")', "=sum(1,,2)"])("some arithmetic expressions", (formula) => {
        const compiledFormula = compiledBaseFunction(formula);
        expect(compiledFormula.execute.toString()).toMatchSnapshot();
    });
...
describe("compile dependencies format", () => {
    ...
    test.each(["=A1", "=A1:B9", "=Sheet34!B3"])("expression with ref return ref dependency", (formula) => {
        const compiledFormula = compiledBaseFunction(formula);
        expect(compiledFormula.dependenciesFormat).toEqual([0]);
    });
    describe("expression with function", () => {
        test.each(["=ANYFUNCTION()", "=ANYFUNCTION(21)", "=ANYFUNCTION(TRUE)", "=ANYFUNCTION(B1)"])("doesn't return formats, whatever args", (formula) => {
            const compiledFormula = compiledBaseFunction(formula);
            expect(compiledFormula.dependenciesFormat.length).toEqual(0);
        });
        test.each(["=RETURNFORMAT()", "=RETURNFORMAT(21)", "=RETURNFORMAT(TRUE)", "=RETURNFORMAT(B1)"])("that has a specific return format return this specific return format, whatever args", (formula) => {
            const compiledFormula = compiledBaseFunction(formula);
    ...
    test.each([
        ...
        ["=-RETURNFORMAT()", [format.specificFormat]],
        ["=-RETURNARGSFORMAT(B1)", [0]],
    ])("expression with unary operator returns dependency format of the right part", (formula, result) => {
        const compiledFormula = compiledBaseFunction(formula);
        expect(compiledFormula.dependenciesFormat).toEqual(result);
    });
    describe("expression with bin operator that has 'ANY' as return format", () => {
        ...
        ])("return dependencies format of the left part if the left par is a string", (formula, result) => {
            const compiledFormula = compiledBaseFunction(formula);
            expect(compiledFormula.dependenciesFormat).toEqual(result);
        });
    });
...
describe("compile functions", () => {
    ...
    describe("with meta arguments", () => {
        ...
        test.each(["=USEMETAARG(A1)", "=USEMETAARG(B2)"])("function call requesting meta parameter", (formula) => {
            const compiledFormula = compiledBaseFunction(formula);
            expect(compiledFormula.execute.toString()).toMatchSnapshot();
        });
        ...
        test("do not care about the value of the cell / range passed as a reference", () => {
            ...
            const rangeA1 = [{ zone: toZone("A1"), sheetId: "ABC" }];
            const rangeA1ToB2 = [{ zone: toZone("A1:B2"), sheetId: "ABC" }];
...
describe("compile functions", () => {
    ...
    describe("check type of arguments", () => {
        test("reject non-range argument when expecting only range argument", () => {
            ...
            expect(() => compiledBaseFunction("=RANGEEXPECTED(42)")).toThrowError("Function RANGEEXPECTED expects the parameter 1 to be reference to a cell or range, not a number.");
            expect(() => compiledBaseFunction('=RANGEEXPECTED("test")')).toThrowError("Function RANGEEXPECTED expects the parameter 1 to be reference to a cell or range, not a string.");
            expect(() => compiledBaseFunction("=RANGEEXPECTED(TRUE)")).toThrowError("Function RANGEEXPECTED expects the parameter 1 to be reference to a cell or range, not a boolean.");
        ...
        test("reject range when expecting only non-range argument", () => {
            for (let typeExpected of ["ANY", "BOOLEAN", "DATE", "NUMBER", "STRING"]) {
            ...
            const m = new Model();
            expect(() => m.getters.evaluateFormula("=?ANYEXPECTED(A1:A2)")).toThrowError("Function ANYEXPECTED expects the parameter 1 to be a single value or a single cell reference, not a range.");
            expect(() => m.getters.evaluateFormula("=BOOLEANEXPECTED(A1:A2)")).toThrowError("Function BOOLEANEXPECTED expects the parameter 1 to be a single value or a single cell reference, not a range.");
            expect(() => m.getters.evaluateFormula("=DATEEXPECTED(A1:A2)")).toThrowError("Function DATEEXPECTED expects the parameter 1 to be a single value or a single cell reference, not a range.");
            expect(() => m.getters.evaluateFormula("=NUMBEREXPECTED(A1:A2)")).toThrowError("Function NUMBEREXPECTED expects the parameter 1 to be a single value or a single cell reference, not a range.");
            expect(() => m.getters.evaluateFormula("=STRINGEXPECTED(A1:A2)")).toThrowError("Function STRINGEXPECTED expects the parameter 1 to be a single value or a single cell reference, not a range.");
            expect(() => m.getters.evaluateFormula("=ANYEXPECTED(A1:A$2)")).toThrowError("Function ANYEXPECTED expects the parameter 1 to be a single value or a single cell reference, not a range.");
            expect(() => m.getters.evaluateFormula("=ANYEXPECTED(sheet2!A1:A$2)")).toThrowError("Function ANYEXPECTED expects the parameter 1 to be a single value or a single cell reference, not a range.");
            expect(() => m.getters.evaluateFormula("=A2:A3")).toThrowError("Function EQ expects its parameters to be single values or single cell references, not ranges.");
            expect(() => m.getters.evaluateFormula("=+A2:A3")).toThrowError("Function UPLUS expects its parameters to be single values or single cell references, not ranges.");
            expect(() => m.getters.evaluateFormula("=A1+A2:A3")).toThrowError("Function ADD expects its parameters to be single values or single cell references, not ranges.");
            expect(() => m.getters.evaluateFormula("=-A2:A3")).toThrowError("Function UMINUS expects its parameters to be single values or single cell references, not ranges.");
            expect(() => m.getters.evaluateFormula("=A1-A2:A3")).toThrowError("Function MINUS expects its parameters to be single values or single cell references, not ranges.");
            expect(() => m.getters.evaluateFormula("=A1+A4*A5:A6-A2")).toThrowError("Function MULTIPLY expects its parameters to be single values or single cell references, not ranges.");
