...
function redoTransformation(toTransform, cancelled) {
...
function undoTransformation(toTransform, cancelled) {
    if (cancelled.position < toTransform.position) {
        return {
            ...toTransform,
            position: toTransform.position - cancelled.value.length,
...
class MiniEditor {
    constructor(undoFn = undoTransformation, redoFn = redoTransformation) {
        ...
        this.history = new SelectiveHistory("initial", this.apply, this.revert, (id) => ({ position: -1, value: "" }), {
            /**
             * Build a transformation function to transform any command as if the execution of
             * a previous `command` was omitted.
             */
            without: (command) => (commandToTransform) => this.undoTransformation(commandToTransform, command),
            /**
             * Build a transformation function to transform any command as if a new `command` was
             * executed before.
             */
            with: (command) => (commandToTransform) => this.redoTransformation(commandToTransform, command),
        });
    ...
    execAfter(insertAfter, instruction) {
...
//# sourceMappingURL=selective_history.test.js.map