...
import { createFullMenuItem } from "../../src/registries";
...
import { getChildFromComponent, makeTestFixture, MockClipboard, mountSpreadsheet, nextTick, Touch, } from "../test_helpers/helpers";
let fixture;
let parent;
let app;
let model;
...
function getSize(menuItemsCount) {
    return {
        width: 200,
...
async function renderContextMenu(x, y, testConfig = {}, width = 1000, height = 1000) {
...
ContextMenuParent.template = xml /* xml */ `
  ...
  `;
ContextMenuParent.components = { Menu };
...
describe("Context Menu", () => {
    ...
    test("menu can be hidden/displayed based on the env", async () => {
        ...
        cellMenuRegistry
            ...
            name: "visible_action",
            sequence: 1,
            isVisible: (env) => env.model.getters.getCell(env.model.getters.getActiveSheetId(), 1, 0).evaluated.value ===
                "b1",
            action() { },
        })
            .add("hidden_action", {
    ...
    test("Menu with icon is correctly displayed", async () => {
        ...
        await simulateClick("div[data-name='root1']");
        expect((_a = fixture.querySelector("div[data-name='root2'] > i")) === null || _a === void 0 ? void 0 : _a.classList).toContain("my-class");
    });
    ...
    test("scroll through the menu with the wheel / scrollbar prevents the grid from scrolling", async () => {
        ...
        const menu = fixture.querySelector(".o-menu");
        // scroll
        menu.dispatchEvent(new WheelEvent("wheel", { deltaY: 300, deltaX: 300, deltaMode: 0, bubbles: true }));
    ...
    test("scroll through the menu with the touch device prevents the grid from scrolling", async () => {
        ...
        menu.dispatchEvent(new TouchEvent("touchstart", {
            bubbles: true,
            cancelable: true,
            touches: [
                new Touch({
        ...
        // move down;
        menu.dispatchEvent(new TouchEvent("touchmove", {
            bubbles: true,
            cancelable: true,
            touches: [
                new Touch({
...
describe("Context Menu - CF", () => {
    test("open sidepanel with no CF in selected zone", async () => {
        ...
        expect(fixture.querySelector(".o-sidePanel .o-sidePanelBody .o-cf .o-cf-preview-list")).toBeTruthy();
        expect(fixture.querySelector(".o-sidePanel .o-sidePanelBody .o-cf .o-cf-ruleEditor")).toBeFalsy();
    });
    test("open sidepanel with one CF in selected zone", async () => {
        ...
        await nextTick();
        await simulateClick(".o-menu div[data-name='conditional_formatting']");
        expect(fixture.querySelector(".o-sidePanel .o-sidePanelBody .o-cf .o-cf-preview-list")).toBeFalsy();
        expect(fixture.querySelector(".o-sidePanel .o-sidePanelBody .o-cf .o-cf-ruleEditor")).toBeTruthy();
    });
