...
import { getChildFromComponent, makeTestFixture, mountSpreadsheet, nextTick, } from "../test_helpers/helpers";
let model;
let fixture;
function focus(index = 0) {
    fixture.querySelectorAll("input")[index].dispatchEvent(new Event("focus"));
}
async function writeInput(index, text) {
    focus(index);
    await nextTick();
...
Parent.template = xml /* xml */ `
  ...
  `;
Parent.components = { SelectionInput };
class MultiParent extends Component {
    setup() {
        useSubEnv({
            model: this.props.model,
        });
...
MultiParent.template = xml /* xml */ `
  ...
  `;
MultiParent.components = { SelectionInput };
async function createSelectionInput(config = {}) {
    model = new Model();
...
describe("Selection Input", () => {
    ...
    test("ctrl + select cell --> add new input", async () => {
        ...
        await nextTick();
        const input = fixture.querySelector(".o-selection-input input");
        ...
        await keyUp("Control");
        const inputs = fixture.querySelectorAll(".o-selection-input input");
    ...
    test("pressing and releasing control has no effect on future clicks", async () => {
        ...
        await simulateClick(".o-cf-add");
        await nextTick();
        let input = fixture.querySelector(".o-selection-input input");
        await simulateClick(input);
        ...
        input = fixture.querySelector(".o-selection-input input");
        expect(input.value).toBe("A2");
        app.destroy();
    });
});
//# sourceMappingURL=selection_input.test.js.map