...
import { createSheet, hideColumns, hideRows, merge, selectCell, setCellContent, setSelection, } from "../test_helpers/commands_helpers";
import { clickCell, rightClickCell, simulateClick, triggerMouseEvent, } from "../test_helpers/dom_helper";
import { getActiveXc, getCell, getCellContent, getCellText } from "../test_helpers/getters_helpers";
import { getChildFromComponent, makeTestFixture, mountSpreadsheet, nextTick, Touch, } from "../test_helpers/helpers";
...
describe("Grid component", () => {
    ...
    describe("keybindings", () => {
        ...
        test("can undo/redo with keyboard", async () => {
            ...
            await nextTick();
            document.activeElement.dispatchEvent(new KeyboardEvent("keydown", { key: "y", ctrlKey: true, bubbles: true }));
            expect(getCell(model, "A1").style).toBeDefined();
        });
        ...
        test("toggle bold with Ctrl+B", async () => {
            ...
            document.activeElement.dispatchEvent(new KeyboardEvent("keydown", { key: "B", ctrlKey: true, bubbles: true }));
            await nextTick();
        ...
        test("toggle Italic with Ctrl+I", async () => {
            ...
            document.activeElement.dispatchEvent(new KeyboardEvent("keydown", { key: "I", ctrlKey: true, bubbles: true }));
            await nextTick();
        ...
        test("can save the sheet with CTRL+S", async () => {
            ...
            document.activeElement.dispatchEvent(new KeyboardEvent("keydown", { key: "S", ctrlKey: true, bubbles: true }));
        ...
        test("can automatically sum with ALT+=", async () => {
            ...
            expect((_a = model.getters.getHighlights()[0]) === null || _a === void 0 ? void 0 : _a.zone).toEqual(toZone("B2:B4"));
        });
        ...
        test("Pressing CTRL+HOME moves you to first visible top-left cell", () => {
            document.activeElement.dispatchEvent(new KeyboardEvent("keydown", { key: "Home", ctrlKey: true, bubbles: true }));
        ...
        test("Pressing CTRL+END moves you to last visible top-left cell", () => {
            document.activeElement.dispatchEvent(new KeyboardEvent("keydown", { key: "End", ctrlKey: true, bubbles: true }));
        ...
        test("Pressing Shift+Space selects the rows of the selection", () => {
            setSelection(model, ["A1:C2"]);
            document.activeElement.dispatchEvent(new KeyboardEvent("keydown", { key: " ", shiftKey: true, bubbles: true }));
            expect(model.getters.getSelectedZone()).toEqual(toZone("A1:Z2"));
        });
        test("Pressing Ctrl+Shift+Space selects the whole sheet", () => {
            document.activeElement.dispatchEvent(new KeyboardEvent("keydown", { key: " ", ctrlKey: true, shiftKey: true, bubbles: true }));
        ...
        test("Pressing Shift+PageDown activates the next sheet", () => {
            ...
            document.activeElement.dispatchEvent(new KeyboardEvent("keydown", { key: "PageDown", shiftKey: true, bubbles: true }));
        ...
        test("Pressing Shift+PageUp activates the previous sheet", () => {
            ...
            createSheet(model, { sheetId: "third", position: 2 });
            expect(model.getters.getActiveSheetId()).toBe("second");
            document.activeElement.dispatchEvent(new KeyboardEvent("keydown", { key: "PageUp", shiftKey: true, bubbles: true }));
            expect(model.getters.getActiveSheetId()).toBe(sheetId);
            document.activeElement.dispatchEvent(new KeyboardEvent("keydown", { key: "PageUp", shiftKey: true, bubbles: true }));
            expect(model.getters.getActiveSheetId()).toBe("third");
        });
...
describe("error tooltip", () => {
    ...
    test("can display error tooltip", async () => {
        ...
        await nextTick();
        expect(document.querySelector(".o-error-tooltip")).not.toBeNull();
        expect((_a = document.querySelector(".o-error-tooltip")) === null || _a === void 0 ? void 0 : _a.parentElement).toMatchSnapshot();
        // moving mouse await
