import { MatchingParenColor, NumberColor, tokenColor, } from "../../src/components/composer/composer";
import { fontSizes } from "../../src/fonts";
import { colors, toZone } from "../../src/helpers/index";
import { activateSheet, createSheet, createSheetWithName, merge, resizeAnchorZone, selectCell, setCellContent, } from "../test_helpers/commands_helpers";
import { clickCell, keyDown, keyUp, simulateClick, triggerMouseEvent, } from "../test_helpers/dom_helper";
import { getActiveXc, getCell, getCellContent, getCellText } from "../test_helpers/getters_helpers";
import { makeTestFixture, mountSpreadsheet, nextTick, startGridComposition, typeInComposerGrid as typeInComposerGridHelper, } from "../test_helpers/helpers";
jest.mock("../../src/components/composer/content_editable_helper", () => require("./__mocks__/content_editable_helper"));
let model;
let composerEl;
let canvasEl;
let fixture;
...
let app;
let cehMock;
function getHighlights(model) {
    return model.getters.getHighlights();
}
async function startComposition(key) {
    const composerEl = await startGridComposition(key);
    // @ts-ignore
    cehMock = window.mockContentHelper;
    return composerEl;
}
async function typeInComposerGrid(text, fromScratch = true) {
...
describe("ranges and highlights", () => {
    test("=+Click Cell, the cell ref should be colored", async () => {
        ...
        expect(
        // @ts-ignore
        window.mockContentHelper.colors["C8"]).toBe(colors[0]);
    });
    test("=SU, the = should be colored", async () => {
        await typeInComposerGrid("=SU");
        // @ts-ignore
        const contentColors = window.mockContentHelper.colors;
    ...
    test("=+Click range, the range ref should be colored", async () => {
        ...
        expect(
        // @ts-ignore
        window.mockContentHelper.colors["B8:C8"]).toBe(colors[0]);
    });
    test.each([
        "A1",
        "$A1",
        ...
        expect(
        // @ts-ignore
        window.mockContentHelper.colors[ref]).toBe(colors[0]);
    });
...
describe("composer", () => {
    ...
    test("typing CTRL+C does not type C in the cell", async () => {
        canvasEl.dispatchEvent(new KeyboardEvent("keydown", { key: "c", ctrlKey: true, bubbles: true }));
        await nextTick();
        expect(model.getters.getCurrentContent()).toBe("");
    });
    test("keyup event triggered after edition end", async () => {
        canvasEl.dispatchEvent(new KeyboardEvent("keydown", Object.assign({ key: "d", bubbles: true })));
        await nextTick();
        ...
        composerEl.dispatchEvent(new KeyboardEvent("keydown", Object.assign({ key: "Enter", bubbles: true })));
        composerEl.dispatchEvent(new KeyboardEvent("keyup", Object.assign({ key: "Enter", bubbles: true })));
        composerEl.dispatchEvent(new KeyboardEvent("keyup", Object.assign({ key: "d", bubbles: true })));
        await nextTick();
        expect(model.getters.getEditionMode()).toBe("inactive");
    });
    ...
    test("Hitting enter on topbar composer will properly update it", async () => {
        ...
        await clickCell(model, "A1");
        const topbarComposerElement = fixture.querySelector(".o-topbar-toolbar .o-composer-container div");
    ...
    describe("change selecting mode when typing specific token value", () => {
        ...
        describe.each(formulas)("typing %s followed by", (formula) => {
            test.each(matchingValues.concat(["("]))("a matching value --> activate 'selecting' mode", async (matchingValue) => {
            ...
            test.each(mismatchingValues.concat([")"]))("a mismatching value --> not activate 'selecting' mode", async (mismatchingValue) => {
            ...
            test.each(matchingValues.concat(["("]))("a matching value & spaces --> activate 'selecting' mode", async (matchingValue) => {
                const content = formula + matchingValue;
            ...
            test.each(mismatchingValues.concat([")"]))("a mismatching value & spaces --> not activate 'selecting' mode", async (mismatchingValue) => {
                const content = formula + mismatchingValue;
                composerEl = await startComposition();
                await typeInComposerGrid(content + "   ");
                expect(model.getters.getEditionMode()).not.toBe("selecting");
            ...
            test.each(mismatchingValues.concat([")"]))("a UNKNOWN token & a matching value --> not activate 'waitingForRangeSelection' mode", async (matchingValue) => {
                const content = formula + "'" + matchingValue;
                composerEl = await startComposition();
                await typeInComposerGrid(content);
                expect(model.getters.getEditionMode()).not.toBe("selecting");
            ...
            test.each(matchingValues.concat([")"]))("a matching value & located before matching value --> activate 'waitingForRangeSelection' mode", async (matchingValue) => {
                composerEl = await startComposition();
            ...
            test.each(mismatchingValues.concat(["("]))("a matching value & located before mismatching value --> not activate 'waitingForRangeSelection' mode", async (mismatchingValue) => {
                composerEl = await startComposition();
                await typeInComposerGrid(mismatchingValue);
                await moveToStart();
                await typeInComposerGrid(formula + ",");
                expect(model.getters.getEditionMode()).not.toBe("selecting");
                expect(composerEl.textContent).toBe(formula + "," + mismatchingValue);
            });
            test.each(matchingValues.concat([")"]))("a matching value & spaces & located before matching value --> activate 'waitingForRangeSelection' mode", async (matchingValue) => {
                composerEl = await startComposition();
                await typeInComposerGrid(matchingValue);
                await moveToStart();
            ...
            test.each(mismatchingValues.concat(["("]))("a matching value & spaces & located before mismatching value --> not activate 'waitingForRangeSelection' mode", async (mismatchingValue) => {
                composerEl = await startComposition();
                await typeInComposerGrid(mismatchingValue);
                await moveToStart();
                await typeInComposerGrid(formula + ",  ");
            ...
            test.each(matchingValues.concat([")"]))("a matching value & located before spaces & matching value --> activate 'waitingForRangeSelection' mode", async (matchingValue) => {
                composerEl = await startComposition();
                await typeInComposerGrid("   " + matchingValue);
                await moveToStart();
                composerEl = await typeInComposerGrid(formula + ",");
            ...
            test.each(mismatchingValues.concat(["("]))("a matching value & located before spaces & mismatching value --> not activate 'waitingForRangeSelection' mode", async (mismatchingValue) => {
                composerEl = await startComposition();
                await typeInComposerGrid("   " + mismatchingValue);
                await moveToStart();
                composerEl = await typeInComposerGrid(formula + ",");
        ...
        test.each([",", "+", "*", ")", "("])("typing a matching values (except '=') --> not activate 'waitingForRangeSelection' mode", async (value) => {
            await startComposition();
    ...
    describe("composer's style depends on the style of the cell", () => {
        test("with text color", async () => {
            ...
            await typeInComposerGrid("Hello");
            const gridComposer = fixture.querySelector(".o-grid-composer");
