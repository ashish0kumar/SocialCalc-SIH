...
import { createChart, createSheet, selectCell, setCellContent, } from "../test_helpers/commands_helpers";
import { clickCell, simulateClick, triggerMouseEvent } from "../test_helpers/dom_helper";
import { makeTestFixture, MockClipboard, mountSpreadsheet, nextTick, startGridComposition, target, typeInComposerGrid, typeInComposerTopBar, } from "../test_helpers/helpers";
...
describe("Spreadsheet", () => {
    ...
    test("typing opens composer after toolbar clicked", async () => {
        ...
        (_a = document.activeElement) === null || _a === void 0 ? void 0 : _a.dispatchEvent(new KeyboardEvent("keydown", { key: "d", bubbles: true }));
        await nextTick();
    ...
    test("can open/close search with ctrl+h", async () => {
        await nextTick();
        document.activeElement.dispatchEvent(new KeyboardEvent("keydown", { key: "H", ctrlKey: true, bubbles: true }));
        await nextTick();
    ...
    test("can open/close search with ctrl+f", async () => {
        document.activeElement.dispatchEvent(new KeyboardEvent("keydown", { key: "F", ctrlKey: true, bubbles: true }));
        await nextTick();
        expect(document.querySelectorAll(".o-sidePanel").length).toBe(1);
        await nextTick();
        document.activeElement.dispatchEvent(new KeyboardEvent("keydown", { key: "F", ctrlKey: true, bubbles: true }));
...
describe("Composer interactions", () => {
    ...
    test("focus top bar composer does not resize grid composer when autocomplete is displayed", async () => {
        ...
        await nextTick();
        const topBarComposer = document.querySelector(".o-spreadsheet-topbar .o-composer");
        const gridComposerContainer = document.querySelector(".o-grid-composer");
    ...
    test("Notify ui correctly with type notification correctly use notifyUser in the env", async () => {
        ...
        Parent.template = xml /* xml */ `<Spreadsheet model="props.model"/>`;
...
describe("Composer / selectionInput interactions", () => {
    ...
    test.each(["A", "="])("Switching from grid composer to selection input should update the highlights and hide the highlight components", async (composerContent) => {
        selectCell(parent.model, "B2");
        OPEN_CF_SIDEPANEL_ACTION(parent.env);
        await nextTick();
